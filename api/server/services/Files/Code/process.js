...
const primeFiles = async (options, apiKey) => {
  ...
  // Pull files registered against this conversation. Two parallel registries:
  //   - persistent_files: user-uploaded files (with db.files rows). Required so
  //     chat-paperclip uploads (no agent_resource flag) reach /mnt/data on the
  //     next code-interpreter call.
  //   - session_files (Track 2): exec-generated files (no db.files row — they
  //     live in code-interpreter's session FS). Forwarded directly to the next
  //     /exec call's files-param so extracted dirs / generated CSVs etc.
  //     survive across sessions.
  // user filter is required hygiene — prevents the cross-user-leak class
  // that hit OneDrive-MCP (2026-05-04). Single findOne projects both arrays.
  let persistentFileIds = [];
  let sessionFileEntries = [];
  if (conversationId) {
    try {
      const convo = await Conversation.findOne(
        { conversationId, user: req?.user?.id },
        { persistent_files: 1, session_files: 1 },
      ).lean();
      persistentFileIds = (convo?.persistent_files ?? []).map((f) => f.file_id);
      sessionFileEntries = convo?.session_files ?? [];
    } catch (err) {
      logger.warn(`[primeFiles] conversation lookup failed: ${err.message}`);
    }
  }
  ...
  // Track 2: append exec-generated files (session_files). They have no
  // db.files row, so they bypass the getFiles() lookup and go directly into
  // the result tuple-list for code-interpreter's files-copy-loop. The
  // copy-loop's copyFileOrDir helper (Phase 1.5) handles directories safely.
  for (const sf of sessionFileEntries) {
    files.push({ session_id: sf.session_id, id: sf.file_id, name: sf.filename });
    if (!toolContext) {
      toolContext = `- Note: The following files are available in the "${Tools.execute_code}" tool environment:`;
    }
    toolContext += `\n\t- /mnt/data/${sf.filename} (generated by previous code run)`;
  }
